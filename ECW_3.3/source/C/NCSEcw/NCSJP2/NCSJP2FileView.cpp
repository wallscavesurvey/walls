...
CNCSError CNCSJP2FileView::Open(wchar_t *pURLPath, bool bProgressiveDisplay, bool bWrite)
{
	...
	if (!bWrite) {
		...
		for (int i = 0; i < sizeof(exts) / sizeof(exts[0]); i++) {
			if (_wcsicmp(pURLPath + wcslen(pURLPath) - wcslen(exts[i]), exts[i]) == 0) {
				bTestECW = false;
				break;
			}
		}
...
CNCSError CNCSJP2FileView::Open(char *pURLPath, bool bProgressiveDisplay, bool bWrite)
{
	...
	if (!bWrite) {
		...
		for (int i = 0; i < sizeof(exts) / sizeof(exts[0]); i++) {
			if (_stricmp(pURLPath + strlen(pURLPath) - strlen(exts[i]), exts[i]) == 0) {
				bTestECW = false;
				break;
			}
		}
...
#ifdef NCSJPC_ECW_SUPPORT
		if (pURLPath && _stricmp(pURLPath + strlen(pURLPath) - strlen(ERS_WAVELET_DATASET_EXT), ERS_WAVELET_DATASET_EXT) == 0) {
	...
	INT32 *pnEPSG)
{
	...
	*pnEPSG = Epsg.GetEPSG(szProjection, szDatum);
	...
	{
		char *pColon;
		if (_strnicmp(szProjection, "epsg:", 5) == 0)
		{
		...
		else if (_strnicmp(szDatum, "epsg:", 5) == 0)
		{
