	...
	UINT16 iComponent)
{
	...
	if (bRet) {
		INT32 nAdd = 0x1 << (pCtx->m_nSsiz - 1);
		INT32 nMax = (0x1 << pCtx->m_nSsiz) - 1;
		...
		case BT_INT16:
			...
			if (CNCSJPCBuffer::sm_SystemInfo.m_bSSE2Present) {
				...
				__m128i mm0 = _mm_set1_epi16((short)nAdd);
				__m128i mm1 = _mm_set1_epi16((short)nMax);
				...
				__m64 mm0 = _mm_set1_pi16((short)nAdd);
				__m64 mm1 = _mm_set1_pi16((short)nMax);
			...
			DCShift_T((INT16*)pBuffer->GetPtr(), (INT16*)pDst->GetPtr(), x, nWidth, nAdd, nMax);
			break;
		case BT_INT32:
			DCShift_T((INT32*)pBuffer->GetPtr(), (INT32*)pDst->GetPtr(), x, nWidth, nAdd, nMax);
			break;
		case BT_IEEE4:
			//				{//FIXME
...
#endif
			DCShift_T((IEEE4*)pBuffer->GetPtr(), (IEEE4*)pDst->GetPtr(), x, nWidth, nAdd, nMax);
			break;
		}
	}
	return(bRet);
