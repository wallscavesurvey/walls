...
#endif
{
	...
	if (_strnicmp("ecwps:", szUrlPath, 6) == 0) {
		bECWPS = TRUE;
		nMatch1 = 0;
	}
	else if (_strnicmp("ecwp:", szUrlPath, 5) == 0) {
		bECWPS = FALSE;
		nMatch1 = 0;
	}
	else {
	...
	//nMatch1 = _strnicmp("ECWP",szUrlPath,4);
	...
	if (nMatch1) {
		...
		*pnFilenameLength = (int)strlen(szUrlPath);
		...
		nMatch1 = _strnicmp(ERS_WAVELET_DATASET_EXT,
		...
		{
			nMatch1 = _strnicmp(JP2Exts[nExtCount],
	...
	char *szUrlPath) //[12]
{
	...
	*ppHeaderMemImage = NULL;
	...
	if (_strnicmp(szUrlPath, "ecwps:", 6) == 0) {
