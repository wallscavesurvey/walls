...
typedef int (* CSF_XFRFCN)(CSH_HDRP hp,UINT typ);
...
struct sCSH_HDR { /*64 bytes*/
    ...
    CSH_HDRP    HashTable;         /*Independent hash table entry comprising*/
                                   /*a separate array*/
    CSH_BUFP    Reserved[4];       /*Fill to 64-byte length*/
};
...
struct sCSH_FILE {          /*Size: 40*/
    ...
    int Handle;             /*Handle returned by dos_OpenFile(), etc. It's actually a ptr to HANDLE*/
	...
	CSF_XFRFCN XfrFcn;      /*Custom IO fcn -- NULL except for NTI_FILEs*/
};
