...
BOOL CFileBuffer::InitBuffer()
{
	ASSERT(!m_pBuffer && m_nBufferSize);
	if(m_nBufferSize && (m_pBuffer=(BYTE *)malloc(m_nBufferSize))) {
...
BOOL CFileBuffer::OpenFail(const char *pszPathName,UINT nOpenFlags,CFileException *pfe)
{
	if (!InitBuffer() || !CFile::Open(pszPathName,nOpenFlags,pfe))
	{
		if(m_nBufferSize) {
			free(m_pBuffer);
			m_pBuffer=NULL;
