...
#ifdef _GDIPLUS_H
#error PolygonClip.h conflicts with GDI+ 
#endif
...
public:
	...
	// CPolygonClip expects a 'normalized' boundary rectangle, meaning
	...
	CPolygonClip(const RectF& boundaries)
	...
	void Clip(pointVector& input, pointVector& clipped)
	{
		...
		for (cpointIterator it = input.begin(); it != input.end(); ++it) m_stageRight.HandleVertex(* it);

		// Do the final step.
		m_stageRight.Finalize();
	}

	void Clip(LPPOINT pt, int cnt, pointVector& clipped)
	{
		clipped.clear();
		m_stageOut.SetDestination(& clipped);
		...
		// Clip each input vertex.
		while(cnt--) {
			m_stageRight.HandleVertex(PointF(pt->x,pt->y));
		}
