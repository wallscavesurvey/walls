...
#else
	if (CreateEx(parent, TBSTYLE_FLAT, WS_CHILD | WS_VISIBLE | CBRS_ALIGN_TOP) && LoadToolBar(TOOLBAR_CONTROL))
	{
#endif
							...
							rect, this, DROPDOWN_FONT ) )
		{
								...
								rect, this, DROPDOWN_SIZE ) )
			{
									...
									rect, this, BUTTON_COLOR ) )
				{
					...
					//m_color.SetCustomText(STRING_CUSTOM);
...
BEGIN_MESSAGE_MAP(CRRECToolbar, CToolBar)
	...
	ON_MESSAGE(CPN_SELCHANGE, OnColorButton)
END_MESSAGE_MAP()
...
LRESULT CRRECToolbar::OnColorButton( WPARAM clr, LPARAM id) 
/* ============================================================
   ...
   ============================================================*/
{
	...
	ASSERT(id==BUTTON_COLOR);

	GetParent()->SendMessage( urm_SETCURRENTFONTCOLOR, 0, ( LPARAM ) clr );
