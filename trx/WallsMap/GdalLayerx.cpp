...
{
	free((LPSTR)m_unitsName);
	free((LPSTR)m_datumName);
	free((LPSTR)m_projectionRef);
	free((LPSTR)m_pathName);
	if(m_pGDAL) {
		GDALClose(m_pGDAL);
	}
}
...
{
	return CLayerSet::TYP_GDAL;
}

BOOL CGdalLayer::ComputeLevelLimit()
{
	m_bLevelLimit=0;
	UINT32 maxDim=max(m_uWidth,m_uHeight)*2;
	UINT32 minDim=min(m_uWidth,m_uHeight)*2; 
...
int __stdcall WallsMap_LoadDLL(const char *path,const char *pHdr)
{
...
{
	ASSERT(!m_pathName && !m_lpBitmapInfo);
	...
	if(!nBands ||
		(pCT && (nBands>2 || m_uColors<2 || m_uColors>256 || m_uDataType!=GDT_Byte)) || !ComputeLevelLimit()) {
			if(!(uFlags&NTL_FLG_SILENT)) {
				CString msg,msg0;
	...
	if(m_uBands!=3) m_crTransColor&=0xFFFFFF; //in case file format changed!
	...
	if(m_bTrans && m_fTransImg[1]>0.0 && m_fTransImg[5]<0.0 && !m_fTransImg[2] && !m_fTransImg[4]) {
	...
	if(m_bTrans) {
		...
		ASSERT(p);
		if(*p) {
			m_projectionRef=_strdup(p);
			OGRSpatialReference oSRS;
			if(oSRS.importFromWkt((LPSTR *)&p)==OGRERR_NONE) {
				if(p=oSRS.GetAttrValue("GEOGCS",0)) {
					bProj=true;
					m_iNad=GetNad(p);
					m_datumName=(strlen(p)<15)?_strdup(p):NULL;
				
					int pbNorth=0;
					m_iZone=(INT8)oSRS.GetUTMZone(&pbNorth);
					ASSERT(m_iZone>=0 && m_iZone<=60);
					if(m_iZone && !pbNorth) m_iZone=-m_iZone;
					if((m_bProjected=(oSRS.IsProjected()==TRUE))) {
						m_fMetersPerUnit=oSRS.GetLinearUnits((LPSTR *)&p);
						m_unitsName=_strdup(p);
						if(!m_iZone) m_iZone=99;
					}
					else {
		...
		if(!bProj) {
			ASSERT(m_iZone==-99 && !m_bProjected && !m_iNad);
...
int CGdalLayer::CopyToDIB(CDIBWrapper *pDestDIB,const CFltRect &geoExt,double fScale)
{
	...
	bool bDibNeeded = true; //(m_hdcMask && m_crTransColorMask && (!m_bUseTransColor || m_crTransColor!=m_crTransColorMask));

	if((fZoom>m_dib_zoom && m_dib_zoom<1.0) || ContainsRect(m_dib_rect,crImage)!=2 || bDibNeeded) //last operation modified original version!
	{
		DeleteMask();
		...
		if(m_uBands==3 && IsUsingBGRE()) ApplyBGRE();

		m_uRasterIOActive=0;
		m_dib_rect=crImage;
		m_dib_zoom=fZoom;
	}
	else if(m_hdcMask && (!m_bUseTransColor || m_crTransColor!=m_crTransColorMask)) {
		DeleteMask();
	}
	...
	//fScale*m_fCellSize = screen pixels to cover in the dimension with the smallest cell length (m_fCellRatio==1) --
 	if(!CopyData(pDestDIB,destOffX,destOffY,fScale*m_fCellSize))
		iRet=0;
