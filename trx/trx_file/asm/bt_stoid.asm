...
TRX_N_Branch	equ	31
TRX_N_DupCutRight equ 32
TRX_N_DupCutLeft equ 64
TRX_N_EmptyRight equ 128

BITS 32

GLOBAL _trx_Btscn   	;fcn
GLOBAL _trx_Btlop
GLOBAL _trx_Btcut
GLOBAL _trx_Btsto

EXTERN  _trx_btscnData 
EXTERN  _trx_btcnt
EXTERN   trx_findResult
EXTERN	_trx_btsfx
EXTERN	_trx_btinc
EXTERN	_trx_btcnt
EXTERN	bt_srch
...
;
;_TRX_BTSCNDATA - Count value of prospective upperbound key, which
...
_trx_Btscn:
	...
	push	ebp
        mov     ecx,dword[_trx_btscnData]	;.CntBound
        or	ecx,ecx
	jz	_scn0
	...
	je	_scnx0
        ...
        mov     esi,[_trx_btscnData+20]	;.ScnOffset
        jmp     short _scn1
...
_scn1:	mov     ebp,[_BTSIZ]	;Use EBP to hold link size
        mov     [_trx_btscnData],ecx	;.Cntbound
        mov     edi,esi
        mov     edx,ecx
        dec     ecx
        cmp     edx,[_trx_btcnt]
        ...
        mov     ebx,[_BTKEY]
        movzx   eax,byte [ebx]
        mov     [_trx_btscnData+16],eax	;.SizBoundKey
        jmp     short _scn2
...
_scn4:  mov     esi,[_trx_btscnData+20]	;.ScnOffset
...
;       BX      = Position of next link
        ...
        mov     [_trx_btscnData+20],ebx
        mov     [_trx_btscnData],edx
...
;       Compute SizDupLast and SizDupLead for duplicate key --
        mov     edi,2
        sub     edi,ebp
        mov     [_trx_btscnData+8],edi	;.SizDupLast,edi
        mov     eax,ebx
        sub     eax,esi
        sub     ecx,[_trx_btcnt]
        jz     _dup1
...
;       Correct for increase in matching prefix --
        sub     eax,[_trx_btsfx]
_dup1:  mov     [_trx_btscnData+4],eax	;.SizDupLead,eax
        jmp     short _srem

;       Compute SizDupLead and SizDupLast for unique key --
_uniq:  xor     eax,eax
        mov     [_trx_btscnData+4],eax	;.SizDupLead,eax
        mov     ecx,[_trx_btcnt]
        cmp     edx,ecx
        jz     _scn5
...
_scn5:  mov     eax,[_trx_btinc]
        add     eax,[_trx_btsfx]
_scn6:  mov     [_trx_btscnData+8],eax	;.SizDupLast,eax
...
_scn8:  sub     ebx,eax
        mov     eax,1
        sub     ebx,[_BTOFF]
        mov     [_trx_btscnData+12],ebx	;.SpcRemain,ebx
...
_trx_Btlop:
		...
		push	esi
		push	edi
        mov     esi,[ebp+8]	;Org_node
        mov     esi,[esi]
        movsx   eax,word [esi+N_SIZLINK]
        movzx   ebx,word [esi+N_FSTLINK]
		add		esi,ebx
        sub     esi,eax
        lodsw           ;AH:AL=pfx:sfx (AH=0)
        mov     edi,[ebp+12]	;Bound_key
        mov     ebx,edi
        movzx   ecx,byte [edi]
        inc     edi
...
; ESI = Addr of first byte of first key in Org_node. AX=length.
        ...
        xchg    cl,al
_lop0:  or	ecx,ecx
	jz      _lopx   ;eliminate special case (null key)
        repz    cmpsb
...
_lop1:  inc     al
        cmp     al,byte [ebx] ;compare new length with original length
        jnb     _lopx
        inc     edi
        mov     cl,byte [edi]
        inc     cl
...
_trx_Btcut:
        ...
        mov     esi,[_BTOFF]
        mov     ecx,[ebp+16]	;Breakpoint
        or      ecx,ecx
        jnz     _btc2
        call    newhi
        jmp     _sto1

newhi:  mov     eax,[ebp+8]		;New_node
        mov     [_BTNOD],eax
        push    esi
        mov     esi,[_BTKEY]
        mov     edi,[ebp+12]	;High_key
        lodsb
        stosb
        movzx   ecx,al
        shr     ecx,1
...
_btc3:  movzx   edx,word [esi+N_NUMKEYS]       ;DX = current # keys (at least one!)
        inc     edx
        movzx   ebx,word [esi+N_FSTLINK]
...
_mov1:  mov     ecx,edx
        ...
        test    byte [edi+N_FLAGS],TRX_N_Branch
        jz      _mov2
...
_mov4:  pop     ebx                ;Restore BX=original node's offset
        cmp     word [ebx+N_NUMKEYS],cx  ;CX==0
        jz      _sto0              ;if no keys remaining
        sub     cx,word [ebx+N_SIZLINK]  ;CX = link size or record size
...
_sto0:  mov     eax,[ebp+16]	;Breakpoint
        test    byte [ebx+N_FLAGS],TRX_N_Branch
        jz      _sto2
...
_sto1:  or      byte [esi+N_FLAGS],TRX_N_DupCutLeft
        mov     edi,[ebp+8]	;New_node
        mov		edi,[edi]
        or      byte [edi+N_FLAGS],TRX_N_DupCutRight
_sto2:
        push    dword [_BTLNK]
        push    dword [_BTKEY]
        push    dword [_BTNOD]
...
_trx_Btsto:
		...
		push	esi
		push	edi
	    mov     edi,[ebp+12]	;New_key
        mov     esi,[ebp+8]	;Old_node
...
; Calculate AX = space required for insertion --
        ...
        movsx   ebp,word  [esi+N_SIZLINK] ;BP = negative link size
        ...
        inc     word [esi+N_NUMKEYS] ;revise key count
        sub     [esi+N_FSTLINK],ax      ;Revise first link's node offset
		movzx	edi,word [esi+N_FSTLINK]
        add     esi,edi
...
_shf1:  mov     edx,edi                 ;Save new key's link position
        ...
        sub     word [esi],ax        ;dec suffix and inc prefix
...
_shf2:  sub     ecx,ebp                 ;Set CX = link or record size
        ...
        mov     esi,[ebp+16]	;New_link
        shr     ecx,1
        rep     movsw
        adc     ecx,ecx
        rep     movsb                   ;Insert link or record
        mov     esi,[ebp+12]	;New_key
...
_ex0:   mov     [_BTSIZ],ebp               ;Save negative link size
		pop		ebp                      ;Restore local BP
        xor     edx,edx
        mov     [_trx_btscnData],edx ;.CntBound,edx  ;Initialize for _trx_Btscn()
        mov     [_BTOFF],esi
		...
		mov		ecx,[ebp+16]	;New_link
		mov		[_BTLNK],ecx
		mov		ecx,[ebp+12]	;New_key
		mov		[_BTKEY],ecx
		mov		ecx,[ebp+8]		;Old_node
		mov		[_BTNOD],ecx
_shfx:
		pop		edi
		pop		esi
