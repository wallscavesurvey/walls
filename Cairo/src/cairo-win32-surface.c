...
_cairo_win32_print_gdi_error (const char *context)
{
	...
	fwprintf (stderr, L"%S: %s", context, (LPWSTR)lpMsgBuf);
				...
				unsigned int		height)
{
	 ...
	 */
	if (src_r.x > (INT32)src_extents.width || src_r.y > (INT32)src_extents.height || //**dmck
	    (src_r.x + (INT32)src_r.width) < 0 || (src_r.y + (INT32)src_r.height) < 0)
	{
