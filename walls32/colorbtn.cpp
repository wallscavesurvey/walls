...
COLORREF CColorBtnDlg::colors[NUM_BTNCOLORS] =
{
    RGB(0,0,0),
    RGB(128,0,0),
    RGB(0,128,0),
...
BYTE CColorBtnDlg::used[NUM_BTNCOLORS] =
{    
...
void CColorBtn::Init(int id,CWnd *pWnd)
{
	VERIFY(SubclassDlgItem(m_id=id,m_pParent=pWnd));
}
...
void CColorBtn::DrawItem(LPDRAWITEMSTRUCT lpd)
{
    ...
    if (lpd->itemState & ODS_FOCUS)	DC.DrawFocusRect(&lpd->rcItem);
...
void CColorBtn::OnClicked() 
{
	...
	if (dlg.DoModal() == IDOK)
    {
		ASSERT(dlg.colorindex>=0 && dlg.colorindex<=NUM_BTNCOLORS);
		if(dlg.colorindex<NUM_BTNCOLORS) {
			currentcolor = CColorBtnDlg::colors[dlg.colorindex];
			InvalidateRect(NULL);
			m_pParent->PostMessage(WM_RETFOCUS,m_id,currentcolor);
		}
		//else execute gradient dialog in CSegView::OnChgColor() --
		else m_pParent->PostMessage(WM_RETFOCUS,m_id,-1);
    }	
}
...
{
    return (AfxGetApp()->WriteProfileBinary("ColorData", "ColorTable",(LPBYTE)CColorBtnDlg::colors,sizeof(COLORREF)*NUM_BTNCOLORS) &&
            AfxGetApp()->WriteProfileBinary("ColorData", "MRU",(LPBYTE)CColorBtnDlg::used,sizeof(BYTE)*NUM_BTNCOLORS));
...
void CColorBtn::Serialize( CArchive& ar )
{
    if (ar.IsStoring())
    {
        ar.Write((void *)CColorBtnDlg::colors,sizeof(COLORREF)*NUM_BTNCOLORS);
        ar.Write((void *)CColorBtnDlg::used,sizeof(BYTE)*NUM_BTNCOLORS);
    }
    else
    {
        ar.Read((void *)CColorBtnDlg::colors,sizeof(COLORREF)*NUM_BTNCOLORS);
        ar.Read((void *)CColorBtnDlg::used,sizeof(BYTE)*NUM_BTNCOLORS);
    }
}
*/
...
BEGIN_MESSAGE_MAP(CColorBtnDlg, CDialog)
    ...
    ON_WM_DRAWITEM()	
	//}}AFX_MSG_MAP
    ON_BN_CLICKED(IDC_GRADIENT,OnGradient)
...
BOOL CColorBtnDlg::OnInitDialog() 
{
	...
	if(!((CColorBtn *)parent)->m_bAllowGrad) {
		GetDlgItem(IDC_GRADIENT)->EnableWindow(FALSE);
		r2.bottom-=22;
	}
    ...
    {
        r2.left = GetSystemMetrics(SM_CXSCREEN) - wd;
    }

    SetWindowPos(NULL,r2.left,r2.top,wd,ht,SWP_NOZORDER);
	...
	GetDlgItem(IDC_OTHER)->SetFocus(); //with return FALSE doesn't work
...
void CColorBtnDlg::OnGradient() 
{
	ASSERT(((CColorBtn *)parent)->m_bAllowGrad==TRUE);
    colorindex = NUM_BTNCOLORS;
    EndDialog(IDOK);
}
...
void CColorBtnDlg::OnOther() 
{
	...
	CColorDialog dlg(((CColorBtn *)parent)->currentcolor, CC_FULLOPEN | CC_ANYCOLOR, this);
...
void CColorBtnDlg::OnOK() 
{
	//Arrive here via ENTER key alone!
	CWnd* pw=GetFocus();
	int i;
	if(pw && (i=pw->GetDlgCtrlID())>=IDC_COLOR1 && i<=IDC_GRADIENT) {
		if(i<=IDC_COLOR20) OnColor(i);
		else if(i==IDC_OTHER) OnOther();
		else OnGradient();
	}
