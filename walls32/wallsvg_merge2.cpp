...
#define MAX_STYLES 300	//increased from 150 for bMerging2
...
static NPSTR errstr[]={
  ...
  "Unsupported element type in predefined layer",
...
#define NUM_SYMTYPES 5
static int iSymType;
enum e_symtype {S_TEXT=1,S_TSPAN,S_TEXTPATH,S_USE,S_IMAGE};
static LPCSTR symType[NUM_SYMTYPES+1]={NULL,
	"text","tspan","textPath","use","image"
};
...
static int getCTM(LPCSTR *attr,SVG_CTM *ctm)
{
	...
	if(p=getAttrVal(attr,bInsideGradient?"gradientTransform":"transform")) {
...
static BOOL getSymPathCTM(LPCSTR *attr,bool bText)
{
	...
	if(!isInside(&pt.x)) return FALSE; //shouldn't happen if symbol
...
static int save_style(char **style,LPCSTR st)
{
	...
	for(i=0;i<MAX_STYLES;i++) {
		if(!style[i]) {
			if(!(style[i]=(char *)malloc(strlen(st)+1))) i=MAX_STYLES;
			else {
				strcpy(style[i],st);
				style[i+1]=NULL;
...
static void outSavedTextElement()
{
	...
	gz_printf(">%s</text>\r\n",trim_refbuf());
#ifdef _DEBUG
	if(!strcmp(refbuf,"Many dead animals.")) {
		int jj=0;
	}
#endif
}
...
static void out_veccolor_entity(char *pColor,int i)
{
	...
	if(pColor && pvecColor) memcpy(pColor,(*pvecColor)[i].char6,6);
	out_entity_line(name,MrgBuff);
}
...
{
	if(!pvecColor) {
		pvecColor=new VEC_COLOR(vecColor0);
		numVecColors=0;
	}
	VEC_COLOR &vc=*pvecColor;
	int i=numVecColors;
	while(i--) if(!memcmp(vc[i].char6,p,6)) return i;
	{
		CHAR6 pc;
		memcpy(pc.char6,p,6);
		vc.push_back(pc);
...
static void free_merge_arrays(int e)
{
	...
	if(numVecColors) {
		numVecColors=0;
		delete pvecColor; pvecColor=NULL;
	}
...
DLLExportC char * PASCAL ErrMsg(int code)
{
  ...
  if(code>SVG_ERR_ABORTED && code<=SVG_ERR_WRITE) {
	sprintf(argv_buf,"%s file %s",(LPSTR)errstr[code],trx_Stpnam(pSVG->svgpath));
	  ...
	  int len=0;
	  if(fpmrg) len=sprintf(argv_buf,"File %s - ",trx_Stpnam(mrgpath));
...
static int write_svg(void)
{
	...
	if(bGzout && !bMerge2) {
		if(!(gzout=gzopen(pathxml,"wb"))) return SVG_ERR_SVGCREATE;
		fpout=0;
	}
	else {
		if(!(fpout=fopen(pathxml,"wb"))) return SVG_ERR_SVGCREATE;
		gzout=0;
	}
...
static int process_path(LPCSTR *attr)
{
	...
	if(bWriting) {
		...
		if(p=getStyleAttr(attr)) {
			...
			else tp=save_style(iLevelSym?groupstyle:pathstyle,p);
		}
		else tp=MAX_STYLES;
...
static int GetGroupFlg(LPCSTR name)
{
	...
	if(IsW2dGroup(name)) {
		...
		for(i=0;i<NUM_W2DNAMES;i++) {
			...
			if(j<=(int)strlen(name) && !memicmp(name,w2d_name[i],j) && (!name[j] || name[j]=='_' && isdigit(name[j+1]))) {
				iGroupFlg=w2d_flg[i];
				iGroupID=i;
				return 1;
			}
...
static void scanw_startElement(void *userData, LPCSTR el, LPCSTR *attr)
{
...
#ifdef _DEBUG
	  int jj=0;
	  if(strcmp(el,"g") && strcmp(el,"symbol")) {
		 jj++;
	  }
#endif
  ...
  if(!strcmp(el,"g")) {
	  ...
	  if(!iLevelSym && (iGroupFlg&FLG_WALLS_DETAIL_MSK) &&
		  (bPathCTM || !getAttrVal(attr,"id") || isFilterPresent(attr))) {
	  ...
	  else if(!iLevelSym || !bSkipSym) outStartElement(el,attr,false);
	  return;
  }
...
static void scanv_startElement(void *userData, LPCSTR el, LPCSTR *attr)
{
  ...
  #ifdef _DEBUG
		pVal=getAttrVal(attr,"id");
		if(pVal && !strcmp("xxx",pVal)) {
			bool bb=bMerging2;
		}
#endif
  ...
  if(!iLevel) {
	//We are not already in a recognized group -- 
	if(!strcmp(el,"g") && (pVal=getAttrVal(attr,"id"))) {
		if(!GetGroupFlg(pVal)) {
  ...
  if(iLevel<0) {
	  if(iLevel==-3) {
		  ...
		  if(!strcmp(el,"use"))
			  save_symname(attr);
		  if(pVal=getStyleAttr(attr)) {
			  save_style(legendstyle,pVal);
		  }
		  return;
	  }
	  //processing reference --
	  if(iLevel==-2) {
		  if(fTitleSize<=0.0 && !strcmp(el,"tspan") && (pVal=getStyleAttr(attr)))
			  fTitleSize=getFontSize(pVal);
		  return;
	  }
	  if(!strcmp(el,"text")) {
		  if(bUseStyles && fTitleSize<=0.0 && (pVal=getStyleAttr(attr)))
			  fTitleSize=getFontSize(pVal);
		  //now look for character data --
		  iLevel=-2;
		  bInTextElement=1;
	  ...
	  if(!strcmp(el,"g")) {
		  ...
		  if(!iLevelSym && (iGroupFlg&FLG_WALLS_DETAIL_MSK) &&
			  ...
			  if(bUseLabelPos && (iGroupFlg&(FLG_LABELS+FLG_NOTES))!=0 &&
				!iLevelID && (pVal=getAttrVal(attr,"id"))) {
				//Station name ID should already be in index, store record in labelRec --
				//Correct for AI id renaming bug!!
				if(pVal[strlen(pVal)-1]=='_') {
					trim_idsuffix((char *)pVal);
				}
				if(*pVal && getLabelRec(pVal)) iLevelID=iLevel;
			  }
		  }
	  ...
	  if(iGroupFlg&FLG_VECTORS) {
		  ...
		  if(GetPathType(el)) {
			  ...
			  switch(GetSymType(el)) {
				  ...
				  case S_TSPAN :
					  if(bInTextPath) {
						  if((pVal=getStyleAttr(attr))) save_style(textstyle,pVal); 
						  break;
					  }

				  case S_IMAGE :
					  if(bInTextElement && (pVal=getStyleAttr(attr)) &&
						  (iSymType!=S_TEXT || strcmp(pVal,strStyleIgnored)))
						   save_style(iLevelSym?groupstyle:textstyle,pVal);
					  break;
		  ...
		  if((pVal=getStyleAttr(attr)) && (iSymType!=S_TEXT||strcmp(pVal,strStyleIgnored)))
			  save_style(legendstyle,pVal);
		  ...
		  if(iGroupFlg&(FLG_LABELS+FLG_NOTES)) {
		  ...
		  else if(pVal=getStyleAttr(attr)) {
			  COLORREF *pc=NULL;
			  LPCSTR pTyp;
			  switch(iGroupID) {
				case ID_BACKGROUND: {
				...
				case ID_GRID:		if(numGridThick<2) {
										el=getStyleValPtr(pVal,st_stroke_width);
				...
				case ID_FRAME: 		if(el=getStyleValPtr(pVal,st_stroke_width)) {
			  ...
			  if(pc && *pc==(COLORREF)-1) {
				  if((*pc=getColorAttr(pVal,pTyp))==(COLORREF)-1) {
...
static int scan_merge_file(void)
{
	...
	if(!bMerging2) {
		if(bAdjust) pSVG->fMaxDiff=(float)(max(max_diffx,max_diffy)/mrgScale);
		...
		if(bAdjust) pSVG->fMaxDiff2=(float)(max(max_diffx,max_diffy)/mrgScale);
	...
	for(i=0;i<(int)pathcnt;i++) {
	  if(check_prange(&pathrange[i])) pathstyle_used[pathrange[i].pathstyle_idx]=i+1;
...
static void replace_name(LPSTR lineoff,int lenoldname,LPCSTR newname)
{
	...
	for(int i=0;i<svg_entityidxcnt;i++) {
		if(!strcmp(oldname,svg_entity_idx[i].name)) {
			...
			if(strcmp(newname,oldname)) {
				replace_name(pid,oldlen,newname);
			}
			break;
		}
	}
...
static bool fix_mrg_name(LPSTR pid)
{
	...
	for(it_name_map it=vNameMap.begin();it!=vNameMap.end();it++) {
		if(!strcmp(oldname,PNAME(it->oldoff))) {
			...
			if(it->newoff>=0)
				replace_name(pid,oldlen,PNAME(it->newoff));
			break;
		}
	}
	return true;
...
#ifndef _DEBUG
		unlink(strcpy(pnamxml,"$1"SVG_TMP_EXT));
#endif
	}
	if(fpin2) {
		fclose(fpin2); fpin2=NULL;
#ifndef _DEBUG
		unlink(strcpy(pnamxml,"$2"SVG_TMP_EXT));
#endif
	}
}
...
int combine_merged()
{
	...
	while((len=fgetline(line1,fpin1))>0) {
		...
		if(!memcmp(line1,sym_prefix,SYM_PREFIX_LEN) && !IsSymMarkerName(line1)) {
	...
	while((len=fgetline(line2,fpin2))>0) {
		...
		if(!memcmp(line2,sym_prefix,SYM_PREFIX_LEN)) {
			...
			if(mapptr) {
				//Name matches, start saving lines instead of writing them
				assert(!vpline2.size());
			...
			if(!strcmp(line2,sym_end)) {
				...
				if(mapptr->vplinecnt==vpline2.size()) {
					...
					if(it2!=vpline2.end()) {
						...
						it2=vpline2.begin();
						strcpy(line2,*it2++);
						replace_name(line2+SYM_PREFIX_LEN,strlen(PNAME(mapptr->oldoff)),PNAME(mapptr->newoff));
						gz_puts(line2);
	...
	//Now write all other non-merged groups, including the w2d_Legend layer in $1.svg
	while((len=fgetline(line1,fpin1))>0) {
		gz_puts(line1);
	}
	if(len<0)
