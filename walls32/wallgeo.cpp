...
static int ComputeDecl(MAGDATA *pD,double lat,double lon)
{
	MODEL *pM;
...
int mag_GetData(MAGDATA *pD)
{
	...
	if(ix<MAG_CVTDATUM) {
		switch(ix) {
			...
			case MAG_CVTUTM2LL		: //returns any LL in range plus conv and decl
				gps_datum=pD->datum;
				UTMtoDeg(pD->zone,pD->east,pD->north,&lat,&lon);
				if(fabs(lat)>90.0 || fabs(lon)>180.0) return MAG_ERR_CVTUTM2LL;
				if(ix==MAG_CVTUTM2LL2 && !(pD->zone2=OutOfZoneConv(&pD->conv,lat,lon,pD->zone)))
				...
				if(ix==MAG_CVTUTM2LL3)
					//no zone check or conv recompute required --
					return 0;
			...
			case MAG_CVTLL2UTM3		: //force utm to specific zone and compute conv but no decl
				lat=getDEG(&pD->lat);
				lon=getDEG(&pD->lon);
				if(lat>84.0 || lat<-80.0 || fabs(lon)>180.0) return MAG_ERR_LLRANGE;
				gps_datum=pD->datum;
				if(ix<MAG_CVTLL2UTM3)
					DegToUTM(lat,lon,&pD->zone,&pD->east,&pD->north);
				else {
					//Force UTM to pD->zone if pD->zone!=0
					pD->zone2=OutOfZoneConv(&pD->conv,lat,lon,pD->zone);
					if(!pD->zone2) return MAG_ERR_ZONERANGE;
					LatLon2UTM(lat,lon,&pD->zone,&pD->east,&pD->north);
					return (pD->zone2!=pD->zone)?MAG_ERR_OUTOFZONE:0;
				}
	  ...
	  if(ix>=MAG_CVTDATUM2) {
		if((ix-=MAG_CVTDATUM2)==pD->datum) return 0;
		gps_datum=pD->datum;
		UTMtoDeg(pD->zone,pD->east,pD->north,&lat,&lon);
		if(fabs(lat)>90.0 || fabs(lon)>180.0) return MAG_ERR_CVTUTM2LL;
	  }
	  ...
	  if(pD->datum!=gps_WGS84_datum) {
		  gps_datum=pD->datum;
		  //Convert to WGS84 --
		  translate(FALSE,&lat,&lon);
	  }
	  gps_datum=ix;
	  //Convert from WGS84 --
	  translate(TRUE,&lat,&lon);
	...
	pD->conv = (float) (sin(Degree*lat)*(lon-lon0));

	if(ix==MAG_CVTLL2UTM2) return 0; //ignore decl
