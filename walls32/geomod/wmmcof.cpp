...
static float wmm_c[13][13],wmm_cd[13][13],wmm_epoch; //data read

/*************************************************************************/

static int wmm_ReadFile(FILE *fd,char *c_str,WMM_ARRAY &c,WMM_ARRAY &cd)
{
  int n,m;
  char c_new[5];
...
{
	WMM_MAG_DATA wmm;
    char buf[81],model[20];
    FILE *fd;

    if(!(fd = fopen("WMM.COF","r"))) return 1;
    if(!fgets(buf,80,fd)) return 1;

	if(wmm_ReadFile(fd,buf,wmm_c,wmm_cd)) return 1;
	fclose(fd);

	if(wmm_Init()) return 1;

	wmm.lat=20.0f;
	wmm.lon=-88.0f;
	wmm.altm=1195.0f;
	wmm.year=2005.0f;
	if(wmm_Calc(&wmm)) return 1;
	wmm.lat=30.0f;
	wmm.lon=-97.0f;
	wmm.altm=0.0f;
	wmm.year=2009.0f;
