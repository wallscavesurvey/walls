/*WALLISEF.C -- Survey file conversion*/
...
static UINT linecount,dircount,dirnest,surveycount,datacount;
static int ln_len;
static BOOL bCombine,bExcluding;
static BOOL bUseUnits;
static char cUnits;
...
enum {SEFD_DAV,SEFD_DVA,SEFD_ADV,SEFD_VDA,SEFD_AVD,SEFD_VAD};
static LPSTR sefd_ordstr[6]={"DAV","DVA","ADV","VDA","AVD","VAD"};
static int sefd_ordidx[6][3]={
...
static int sefd_atdec,sefd_mtdec,sefd_fcdec,sefd_fidec;
...
static BOOL bCPoint,bParen,sefd_revAB,sefd_revVB;
static BOOL lastblank;
static int *sefd_idx;
static int sefd_dec[3];

#ifdef _FIX_DIST
static double sefd_orgdist;
static double sefd_maxdif;
#endif
...
#ifdef _OLD_FLTSTR
static LPSTR NEAR PASCAL fltstr(double dblFloat,int dec)
{
   static char buf[32];
   static double dblInt;
   int len=0;
   
   if(dblFloat==BLANK_VAL) return strcpy(buf,"--");
   ...
   if(!dec) {
	 dblFloat+=0.5;
 	 if(dblFloat>=10000000.0) strcpy(buf+len,"*******");
 	 else wsprintf(buf+len,"%lu",(DWORD)dblFloat);
 	 return buf;
   }
   
   if(dblFloat<5.0/pow(10.0,dec+1)) {
     strcpy(buf+len,"0");
     return buf;
   }

   // Get the fractional part and the integer part of the float
   dblFloat = (1.0+modf(dblFloat,&dblInt)) * pow(10.0,dec) + 0.5;
   
   dec=len+=wsprintf(buf+len,"%lu",(DWORD)dblInt);

   len+=wsprintf(buf+len,"%lu",(DWORD)dblFloat);
   buf[dec++]='.';
   while(len>dec && buf[len-1]=='0') len--;
   if(len==dec) len--;
   buf[len]=0;
   return buf;
}
...
DLLExport int PASCAL ErrMsg(LPSTR buffer,int code)
{
  int len=0;
...
static apfcn_v process_pragma(void)
{
	
	if(cfg_GetArgv(ln+1,CFG_PARSE_ALL) && !strcmp(_strupr(cfg_argv[0]),"VERSION")) {
	  bUseUnits=cfg_argc<2 || strcmp(cfg_argv[1],"92");
	}
}
...
static apfcn_i new_dir(char *title)
{
  ...
  wsprintf(pathtail,"%03uD",++dircount);
  if(!(e=write_prj("BOOK",title)) &&
     !(e=write_prj("NAME",pathname)) &&
...
static apfcn_v get_timestr(void)
{
  ...
  if(!_fstat(_fileno(fp),&st)) {
    ...
    wsprintf(timestr,"%02d:%02d %04d-%02d-%02d",
...
static apfcn_i new_survey(char *title)
{
  ...
  if(!fpts) {
	  ...
	  wsprintf(pathtail,"%04u.SRV",++surveycount);
	  if(write_prj("NAME",pathname) || write_prj("STATUS","8")) return SEF_ERR_WRITE;
...
static apfcn_v append_cc(char *bp)
{
  ...
  for(i=0;i<SIZ_CC;i++) if(sefd_off[sefd_ccid[i]]) {
     ...
     wsprintf(bp," %s=%s%s\n",(LPSTR)sefd_ccnam[i],fltstr(sefd_ccval[i],*sefd_ccdec[i]),
       (LPSTR)units);
  }
}  
...
static apfcn_i process_data(char *p)
{
    ...
    *pnxt=0;
    ...
    if(bParen) {
	    ...
	    *pnxt=0;
	    ...
	    if((UINT)sefd_len[ikey]>SEFD_SIZBF-1 ||
	      sefd_off[ikey]+sefd_len[ikey]>SIZLN) return SEF_ERR_DATAARGS;
	}
	else {
  ...
  if(bCPoint) {
	  ...
	  if((az_off=sefd_off[SEFD_FAZI])==0) {
	    if((az_off=sefd_off[SEFD_BAZI])==0) return SEF_ERR_DATAARGS;
	    sefd_bazi=-1;
	    //sefd_off[SEFD_FAZI]=sefd_off[SEFD_BAZI];
	    //sefd_len[SEFD_FAZI]=sefd_len[SEFD_BAZI];
	  }
	  else sefd_bazi=sefd_off[SEFD_BAZI]!=0;
	
	  if((inc_off=sefd_off[SEFD_FINC])==0) {
	    if((inc_off=sefd_off[SEFD_BINC])==0) return SEF_ERR_DATAARGS;
	    sefd_binc=-1;
	    //sefd_off[SEFD_FINC]=sefd_off[SEFD_BINC];
	    //sefd_len[SEFD_FINC]=sefd_len[SEFD_BINC];
	    //sefd_off[SEFD_BINC]=0;  /*not required*/
	  }
	  else sefd_binc=sefd_off[SEFD_BINC]!=0;
	
	  if(sefd_off[SEFD_DIST]<az_off) {
  ...
  if(bCPoint) {
	  wsprintf(bf,"#Units %s Grid=0\n;CPoint Reference: %s\n",
	     (LPSTR)sefd_units,(LPSTR)sefd_cpunits);
	  ...
	  wsprintf(bf,"#Units %s Order=%s Decl=%s\n",
	     (LPSTR)sefd_units,(LPSTR)sefd_ordstr[sefd_ord],fltstr(sefd_decl,2));
	  ...
	  for(ikey=0;ikey<4;ikey++) if(sefd_ccunits[ikey]&&sefd_ccval[ikey]!=0.0) break;
	  if(ikey<4 && EOF==
	    fputs(";!CAUTION: Units for angular corrections may be wrong. Degrees assumed.\n",fpts)) return SEF_ERR_WRITE;
...
static apfcn_i process_key(int typ)
{
  ...
  switch(typ) {
    ...
    case SEF_ZONE	  : if(bCPoint) {
    					  if(*sefd_cpunits) strcat(sefd_cpunits,", ");
    					  strcat(sefd_cpunits,"Zone ");
    					  strcat(sefd_cpunits,p);
    					}
    					break;
    					
    case SEF_UNITS    : _strupr(p);
						if(!bCPoint) {
							if(bUseUnits) cUnits=*p;
							break;
						}
    					strcpy(sefd_cpunits,p);
     					if(toupper(*p)!='F') *p='m';
    ...
    case SEF_MTDEC    : if((sefd_mtdec=atoi(p))>SEF_MAXDEC||sefd_mtdec<0) retval=SEF_ERR_MAXDEC;
						#ifdef _FIX_DIST
                        else sefd_maxdif=5.0/pow(10.0,sefd_mtdec);
                        #endif
                        break;
...
/*
static double PASCAL rev_azi(double d)
{
  d+=180.0;
  if(d>360.0) d-=360.0;
  return d;
...
static VOID PASCAL ElimSpc(char *p0)
{
    //Convert spaces to underlines and long names to prefixed names.
    //Also substitute for certain WALLS special characters.
    char *p=p0;
    int i;
...
#ifdef _FIX_DIST
static apfcn_i adjust_dist(double h)
{
  double va=h*sin(U_DEGREES*sefd_val[SEFD_FINC]);

  sefd_orgdist=sefd_val[SEFD_DIST];
  h=sefd_orgdist*sefd_orgdist+va*va-h*h;
  if(h<0.0 || fabs(sefd_orgdist-(va+=sqrt(h)))<sefd_maxdif) return FALSE;
  sefd_val[SEFD_DIST]=va;
  return TRUE;
}
#endif
...
static apfcn_i process_vector(void)
{
  ...
  *sefd_type=0;
  ...
  if(bCPoint) {
	  ...
	  if(!bLrud) {
		  ...
		  if(sefd_htcorrect) {
		    ...
		    else htcorr=BLANK_VAL;
		    if(sefd_htcorrect&2) iab=get_number(SEFD_IAB);
		    else iab=BLANK_VAL;
		    if(sefd_htcorrect&4) tab=get_number(SEFD_TAB);
		    else tab=BLANK_VAL;
		    
		    #ifdef _FIX_DIST
		    if(sefd_tmeth!=SEFD_IT && sefd_htcorrect>1) {
		
		      switch(sefd_tmeth) {
		        case SEFD_SS: bAdjustDist=adjust_dist(tab-iab);
		                      break;
		        case SEFD_IS: bAdjustDist=adjust_dist(tab);
		                      break;
		        case SEFD_ST: bAdjustDist=adjust_dist(-iab);
		                      break;
		      }
		    }
		    #endif
      ...
      if(!bLrud) {
		  for(i=0;i<3;i++) len+=wsprintf(bf+len,"\t%s",davstr(sefd_idx[i],sefd_dec[i]));
		  #ifdef _FIX_DIST
			  if(sefd_htcorrect && fabs(htcorr)>=sefd_maxdif)
			    len+=wsprintf(bf+len,"\t%s",fltstr(htcorr,sefd_atdec));
		  #else
			  if(sefd_htcorrect) {
			    if(htcorr!=BLANK_VAL) {
			      if(sefd_tmeth==SEFD_ST) tab-=htcorr; //don't include htcorr in slope distance adj
			      else iab+=htcorr;
			    }
			    if(iab!=BLANK_VAL || tab!=BLANK_VAL) len+=append_height(bf+len,iab);
			    if(tab!=BLANK_VAL || sefd_htcorrect>1 && iab!=BLANK_VAL) len+=append_height(bf+len,tab);
			  } 
		  #endif
      }
  ...
  if(*sefd_type) len+=wsprintf(bf+len,"\t#S %s",(LPSTR)sefd_type);
  
  #ifdef _FIX_DIST
  if(bAdjustDist) len+=wsprintf(bf+len,"\t;!(%s Dist=%s)",
	(LPSTR)seftmeth[sefd_tmeth],fltstr(sefd_orgdist,sefd_mtdec));
  #endif
...
DLLExport int PASCAL Import(LPSTR lpSefPath,LPSTR lpPrjPath,LPFN_IMPORT_CB pCB)
{
  int e;
  ...
  trx_Stcext(sefpath,lpSefPath,"SEF",_MAX_PATH);
  trx_Stcext(prjpath,lpPrjPath,"PRJ",_MAX_PATH);

  if((fp=fopen(sefpath,"r"))==NULL) return SEF_ERR_SEFOPEN;
  get_timestr();

  if((fpto=fopen(prjpath,"w"))==NULL) return errexit(SEF_ERR_CREATE);

  /*Set pointer prjpath tail --*/
  pathname=pathtail=_NCHR(trx_Stpnam(prjpath));
  for(e=0;e<4;e++,pathtail++) if(*pathtail=='.') break;

  sefname=_NCHR(trx_Stpnam(sefpath));
  ...
  trx_Bininit((DWORD *)sefdtypes,&sefd_numtypes,comp_sefd);
  linecount=surveycount=dircount=dirnest=datacount=0;
  bCPoint=FALSE;
  bUseUnits=TRUE;
    ...
    if(*ln=='#') {
      ...
      if(e) return errexit(e);
		...
		if(*ln==' ') {
		  ...
		  if(*ln=='%') process_pragma();
		  else process_comment(ln);
		}
	}

    if((linecount%SEF_CB_INCREMENT)==0 && !pCB(linecount)) return errexit(SEF_ERR_ABORTED);
  }
