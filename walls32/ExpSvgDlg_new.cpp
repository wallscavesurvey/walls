...
int CExpSvgDlg::CheckData()
{
	...
	if(m_bShpWalls||m_bShpDetail||m_bShpLegend) {
		...
		m_pSVG->mrgpath=(LPCSTR)m_MergePath;
	}
...
void CExpSvgDlg::Browse(UINT id,UINT ids)
{
	BrowseFiles((CEdit *)GetDlgItem(id),m_pathname,".svgz",IDS_SVG_FILES,ids);
}

void CExpSvgDlg::OnBrowse() 
{
	Browse(IDC_2DPATH,IDS_SVG_BROWSE);
}

void CExpSvgDlg::OnBrowse2() 
{
	Browse(IDC_MERGEPATH,IDS_SVG_BROWSE2);
}
...
BOOL CExpSvgDlg::OnInitDialog() 
{
	...
	GetDlgItem(IDC_MERGEPATH)->EnableWindow(m_bShpWalls||m_bShpDetail||m_bShpLegend);
	GetDlgItem(IDBROWSE2)->EnableWindow(m_bShpWalls||m_bShpDetail||m_bShpLegend);
...
{
	BOOL bChk=Checked(IDC_SHP_MERGEDCONTENT) ||
