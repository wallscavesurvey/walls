...
#define WINVER	0x0600	// Target Windows Vista //was 0x0600 dmck
#ifdef _WIN32_WINNT
#undef _WIN32_WINNT
#endif
...
void CRectTracker::Draw(CDC* pDC) const
{
	...
	if ((m_nStyle & (dottedLine|solidLine)) != 0)
	{
		if (m_nStyle & dottedLine)
			...
			pOldPen = (CPen*)pDC->SelectStockObject(WHITE_PEN); /*DMcK*/
	...
	if ((m_nStyle & (resizeInside|resizeOutside)) != 0)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			if (mask & (1<<i))
			{
				...
				pDC->FillSolidRect(rect, RGB(255, 255, 255));  /*DMcK*/
			}
		}
	}
