...
int CDECL LoadCMsg(char *buf,int sizBuf,UINT nIDS,...)
{
	...
	*buf=0;
    ...
    int iRet=_vsnprintf(buf,sizBuf,format,marker);
    va_end(marker);
    return iRet;
}
...
int CDECL CMsgBox(UINT nType,char *format,...)
{
  ...
  _vsnprintf(buf,512,format,marker);
  va_end(marker);
...
void CDECL StatusMsg(UINT nIDS,...)
{
  ...
  va_list marker;
  va_start(marker,nIDS);
  _vsnprintf(buf,128,format,marker);
  va_end(marker);
   ...
   int numFilters,CString &strFilter,BOOL bOpen,UINT ids_Title,char *defExt)
{
	...
	DWORD flags=lFlags;
	if(ids_Title==IDS_SVG_BROWSE2) {
		flags|=(OFN_ALLOWMULTISELECT | OFN_HIDEREADONLY | OFN_ENABLESIZING | OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_EXPLORER | OFN_LONGNAMES);
	}
	else flags|=(OFN_HIDEREADONLY | OFN_ENABLESIZING);

	//CFileDialog dlgFile(bOpen);  //TRUE for open rather than save
    CFileDialog dlgFile(bOpen,NULL,NULL,flags);
	CString title;
	VERIFY(title.LoadString(ids_Title));

	char path[3*_MAX_PATH+1],initDir[_MAX_PATH];
