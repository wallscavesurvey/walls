...
void CQuickList::DrawItem(int item, int subitem, CDC* pDC)
{
		...
		if(id.m_pXCMB && !(id.m_pXCMB->wFlgs&XC_LISTNOEDIT))
		{
			DrawArrowBox(id,pDC);
		}
}
...
void CQuickList::EditSubItem (int Item, int Column, POINT *pt)
{
	...
	if(id.m_pXCMB && id.m_allowEdit) {
		if((id.m_pXCMB->wFlgs&XC_LISTNOEDIT) || !pt || pt->x > Rect.right-Rect.Height()) {
...
BOOL CQuickList::OnToolTipText(UINT /*id*/, NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	ASSERT(nItem>=0);
	if((nSubItem==0)==(m_nTTInitial<100)) //fieldlist:200 else 64
		return TRUE;

	if(!nItem && m_bHdrToolTip && pt.y==m_hdrHeight) {
		if(nSubItem<=0) return TRUE;
	...
	ToolTipCustomize(m_nTTInitial,m_nTTAutopop,m_nTTReshow,(nItem<0)?&m_hdrToolTip:NULL);
...
void CQuickList::EnableHeaderEx()
{
	...
	if(m_bUsingHeaderEx) {
		ASSERT(m_pCellFont && m_pGridFont==m_HeaderCtrl.GetFont());
		HDITEM hdItem;
		hdItem.mask = HDI_FORMAT;
