...
void CImageLayer::CreateMask(HDC hdc)
{
	...
	COLORREF bkclr=SetBkColor(hdc,m_crTransColorMask=m_crTransColor); //Background color in rgb bitmap mapped to 1 in mask
...
bool CImageLayer::CopyData(CDIBWrapper *pDestDIB,int destOffX,int destOffY,double fZoom)
{
	...
	//if(m_bUseTransColor && !m_hdcMask)	CreateMask(srcDC);
	...
	if(m_bAlpha!=0xFF) {
		...
		if(m_bUseTransColor) { //or, if HALFTONE needed (averaging and fZoom>1), use this code path without applying mask!
			...
			//Black out the blended border of hdcTemp in preparation for a SRCPAINT --
			VERIFY(::PatBlt(m_hdcMask,0,0,srcWidth,srcHeight,DSTINVERT));
			//Now:  0x0000 (border portion of mask), 0xFFFF (interior portion of mask)
			COLORREF clrbk=::SetBkColor(hdcTemp,RGB(255, 255, 255));
			COLORREF clrtx=::SetTextColor(hdcTemp,RGB(0, 0, 0));
			VERIFY(::BitBlt(hdcTemp,0,0,srcWidth,srcHeight,m_hdcMask,0,0,SRCAND));
			::SetTextColor(hdcTemp,clrtx);
			::SetBkColor(hdcTemp,clrbk);
			VERIFY(::PatBlt(m_hdcMask,0,0,srcWidth,srcHeight,DSTINVERT)); //restore mask

			//zero interior of destination --
			ApplyMask(destDC,destOffX,destOffY,destWidth,destHeight);

			//OR the alpablended source with zero borders to destination --
			int mode=(fZoom>1.0 && !(m_pDoc->m_uEnableFlags&NTL_FLG_AVERAGE))?COLORONCOLOR:HALFTONE;
			//int mode=COLORONCOLOR;
			int iPrevMode=::SetStretchBltMode(destDC,mode);
			if(::StretchBlt(destDC,destOffX,destOffY,destWidth,destHeight,hdcTemp,0,0,srcWidth,srcHeight,SRCPAINT))
				bRet=true; //OK!!!
			...
			if(m_hdcMask) DeleteMask();
			if(::AlphaBlend(destDC,destOffX,destOffY,destWidth,destHeight,srcDC,0,0,srcWidth,srcHeight,bf))
				bRet=true;
		}
	}
	else {
		//ASSERT(m_bUseTransColor==(m_hdcMask!=NULL));
		if(m_bUseTransColor) {
			if(m_hdcMask) DeleteMask();
			::TransparentBlt(destDC,destOffX,destOffY,destWidth,destHeight,srcDC,0,0,srcWidth,srcHeight,m_crTransColor&0xffffff);
			return true;
		}

		//if hdcmask, OR pDestDC (assuming black==transparent) to the destination --

 		int mode=(fZoom>1.0 && !(m_pDoc->m_uEnableFlags&NTL_FLG_AVERAGE))?COLORONCOLOR:HALFTONE;
		//int mode=COLORONCOLOR;
		int iPrevMode=::SetStretchBltMode(destDC,mode);
		if(::StretchBlt(destDC,destOffX,destOffY,destWidth,destHeight,srcDC,0,0,srcWidth,srcHeight,SRCCOPY))
		    bRet=true;
		::SetStretchBltMode(destDC,iPrevMode);
	}
	return bRet;
