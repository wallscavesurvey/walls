...
static WMM_COFDATA wmm_cofdata;
static int bWMM;
static int bUseWMM;

static int get_wmm_cofdata(FILE *fd, char *c_str, float &epoch, WMM_ARRAY &c, WMM_ARRAY &cd)
{
	int n, m;
	char c_new[5], model[80];
	float gnm, hnm, dgnm, dhnm;

	if (sscanf(c_str, "%f%s", &epoch, model) != 2) return 1;
...
FLTYP gh1[196];
FLTYP gh2[196];
FLTYP gha[196];                                  /* Geomag global variables */
FLTYP ghb[196];
...
int readshc(int nmax)
{
	...
	for (nn = 1; nn <= nmax; ++nn)
	{
		for (mm = 0; mm <= nn; ++mm)
		{
			fgets(inbuff, 3, stream); //reads two chars
			//inbuff[3]='\0';
			sscanf(inbuff, "%d", &n); //was m and then n
			fgets(inbuff, 4, stream);
			//inbuff[3]='\0';
			sscanf(inbuff, "%d", &m);
			if (++num_shc > MAXSHC || (nn != n) || (mm != m)) {
				return -1;
			}
			fgets(inbuff, MAXREAD - 4, stream); //was MAXREAD-4
...
int main(int argc, char**argv)
{
	...
	FLTYP zdot;
	FLTYP dmin, imin;
	char  mdfile[MAXREAD + 2];
	char  inbuff[MAXINBUFF + 1];
	...
	if (strstr(mdfile, "WMM")) bUseWMM = 1;
	if (strchr(mdfile, '.')) {
	...
	while (fgets(inbuff, MAXREAD, stream)) {
		...
		if (strlen(inbuff) != RECL || strncmp(inbuff, "   ", 3)) {
			if (!bUseWMM) break;
			if (!get_wmm_cofdata(stream, inbuff, wmm_cofdata.epoch, wmm_cofdata.c, wmm_cofdata.cd)) {
				bWMM = 1;
				break;
			}
		...
		char mName[12]; mName[11] = 0;
		sscanf(inbuff, "%s%f%d%d%d%f%f%f%f", mName, &pM->epoch,
			&pM->max1, &pM->max2, &pM->max3, &pM->yrmin,
			&pM->yrmax, &pM->altmin, &pM->altmax);

		if (strlen(mName) == 8) {
			memcpy(pM->name, mName + 4, 4);
		}
		else {
			assert(strlen(mName) == 6);
			memcpy(pM->name, "19", 2);
			memcpy(pM->name + 2, mName + 4, 2);
		}
		pM->name[4] = 0;
	...
	if (bWrite) {
		int size = 2 * sizeof(int) + sizeof(num_model) + sizeof(MODEL)*num_model + sizeof(SHC)*num_shc;
		int offWMM = 0;
		if (bWMM) {
			offWMM = size;
			size += sizeof(wmm_cofdata);
		}
		strcat(mdfile, ".BIN");
		if (!(stream = fopen(mdfile, "wb")) ||
			fwrite(&size, sizeof(int), 1, stream) != 1 ||
			fwrite(&offWMM, sizeof(int), 1, stream) != 1 ||
			...
			(bWMM && fwrite(&wmm_cofdata, sizeof(wmm_cofdata), 1, stream) != 1) ||
			fclose(stream))
		{
