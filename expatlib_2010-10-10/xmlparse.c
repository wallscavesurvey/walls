...
#endif
	enum XML_PredefinedEntityParsing m_predefinedEntityParsing;
	enum XML_GeneralEntityParsing m_generalEntityParsing;
} Parser;
...
#define predefinedEntityParsing (((Parser *)parser)->m_predefinedEntityParsing)
#define generalEntityParsing (((Parser *)parser)->m_generalEntityParsing)
#ifdef XML_DTD
...
XML_ParserCreate_MM(const XML_Char *encodingName,
	...
	namespaceSeparator = '!';
	predefinedEntityParsing = XML_PREDEFINED_ENTITY_PARSING_ALWAYS;
	generalEntityParsing = XML_GENERAL_ENTITY_PARSING_ALWAYS;
#ifdef XML_DTD
	parentParser = 0;
...
void XML_SetPredefinedEntityParsing(XML_Parser parser, enum XML_PredefinedEntityParsing parsing)
{
	predefinedEntityParsing = parsing;
}

void XML_SetGeneralEntityParsing(XML_Parser parser, enum XML_GeneralEntityParsing parsing)
{
	generalEntityParsing = parsing;
}
		...
		*eventEndPP = next;
		...
		case XML_TOK_ENTITY_REF:
		{
			...
			if (ch) {
				if (characterDataHandler) {
					//DMcK=== if (entity) {
					if (predefinedEntityParsing) characterDataHandler(handlerArg, &ch, 1);
					else characterDataHandler(handlerArg, s, next - s);
				}
				else if (defaultHandler) reportDefault(parser, enc, s, next);
				break;
			}

			//DMcK===
			if (!generalEntityParsing) {
				if (characterDataHandler) characterDataHandler(handlerArg, s, next - s);
				else if (defaultHandler) reportDefault(parser, enc, s, next);
				break;
			}
			...
			if (!entity) {
				if (dtd.complete || dtd.standalone) return XML_ERROR_UNDEFINED_ENTITY;
				if (defaultHandler) reportDefault(parser, enc, s, next);
				break;
			}
	...
	for (;;) {
		...
		case XML_TOK_ENTITY_REF:
		{
			...
			XML_Char ch = XmlPredefinedEntityName(enc,
				...
				next - enc->minBytesPerChar);
			if (ch) {
				/*DMcK ============================= if (!poolAppendChar(pool, ch))*/
				if ((predefinedEntityParsing && !poolAppendChar(pool, ch)) ||
					(!predefinedEntityParsing && !poolAppend(pool, enc, ptr, next)))
					return XML_ERROR_NO_MEMORY;
				break;
			}
