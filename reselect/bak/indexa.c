	...
	for(;*psrc;psrc++) {
		...
		*++p=*psrc;
		...
		if(p-pkey==MAX_NAMEKEYLEN) {
			logmsg(cKey, "Truncated 60-chr key placed in name index");
			break;
		}
	}
	...
	if(p==pkey) {
		...
		if(!pfxcmp(p+1,"ANONYMOUS")) {
			*p=9; p[10]=0;
			...
			if(p>pkey) {
				p++;
				if(!pfxcmp(p,"PSEUD") || !pfxcmp(p, "EDITOR") || !pfxcmp(p, "DIRECTOR") ||
					!pfxcmp(p, "MODERATOR")) {
					*--p=0;
					*pkey=(byte)(p-pkey-1);
				}
			...
			if(*pkey) {
				...
				if(trx_Find(trx,pkey))
					logmsg(pkey+1, "Indexed as pseudonym or association");
			}
		}
	}
...
void index_author(int trxno,UINT refno,char *pRef)
{
	...
	while(len && (p0[len-1]==',' || p0[len-1]=='.')) len--;

	p0[len]=0; //remove date
	p=p0;

	//Author is an association if name following "AND" is one word
	px=strstr(p," AND ");
	if(px && strchr(px+5,' ')) px=NULL;

	if(px || !(px=strrchr(p,','))) {
		//Treat as association --
		if(!pfxcmp(p,"ANONYMOUS")) {
			p[9]=0;
			add_author_key(p);
			return;
		}
...
_retry:
        nComps=0;
		while(px=strrchr(p,'.')) {
		   if(!nComps) logmsg(p,"Name has nultiple components");
		   nComps++;
		   *px++=0;
		   px=skip_space_types(px);
		   if(strlen(px)>=3) {
			   add_author_key(px);
		   }
		   else {
			   logmsg(px,"Component length < 4 chars (not indexed)");
		   }
		}
		...
		if(strlen(p)>=3) {
			...
			logmsg(p,"Component length < 4 chars (not indexed)");
		}
		return;
	}

	//*px is last comma in author string
	if(*++px==' ') px++;
	if(!pfxcmp(px, "EDITOR") || !pfxcmp(px, "DIRECTOR") || !pfxcmp(px, "MODERATOR")) {
		if(*--px!=',') px--;
		*px=0;
	}

	if(!*p) goto _err;
	if(!(px=strchr(p,','))) goto _retry;

	//px points to 1st comma in  author string at p==p0
	if((p0=strchr(p,' ')) && p0<p) {
		logmsg(p,"Note: 1st author's name contains space");
	}
	...
	while(*p) {
		...
		for(;*p;p++) {
			...
			if(*p=='&') {
				if(!memicmp(p,"&nbsp;",6)) {
					p+=5;
					if(bLastSpc || is_xspace(p[1])) continue;
					bLastSpc=TRUE;
					c=' ';
					goto _next;
				}
