...
static bool bNoInc, bLast_NoInc, bConvert, bDateFirst, bDateSeen, bDeclSeen;
static LPSTR *pCC;
static int numCC;
static int maxCC;
...
enum e_arg {
	ARG_NONE, ARG_EQUALS, ARG_PARAM, ARG_NUMERIC, ARG_MACRODEF,  /*Not keys*/
...
static SRV_LRUD lruddat, lruddat_pending;
static BOOL bLrudPending, bErrTwoNames;
...
static BOOL bUseDates, bFixVtVert, bFixVtLen, bLrudOnly, bFixed, bRMS;
...
static char *errmsg[] =
{
	...
	//wallexp.dll only --
	"Cannot initialize SEF file",	//66
	"Prefixed name length > 20 chars. Try again after enabling prefix length reduction",
	EXP_VERSION_STR
};
...
static void parse_type(LPSTR pseg)
{
	...
	*/
	LPSTR p = trx_Stpnam(pseg); //past last slash
	int i = 0;
	for (; i < 4; i++) {
...
static int ParseLineCommand(char *line)
{
	...
	*NoteBuf = 0;
	if (bFixed) {
		...
		if (!bLrudOnly && (*line == SRV_CHAR_LRUD || *line == '*')) {
			//two station names precede the LRUD expression
			bLrudOnly = 2;
		}
		p = strchr(line, SRV_CHAR_COMMAND);
	}
...
{
	//fills pVec->nam[i], pVec-pfx[i], and vec_pfx[i] (full prefix)
...
{
	//uses pVec->nam[i], pVec->pfx[i], and vec_pfx[i]
	char nam[EXP_MAX_NAMLEN + 4];
	char *p;
...
static apfcn_i ParseCommand(int cmd, char *p)
{
	...
	/*UNITS - Translate keywords and process possible RESET argument --*/
	...
	for (i = 0; i < cfg_argc; i++) {
		...
		case ARG_DECL: if (bDateSeen) bDateFirst = true;
			bDeclSeen = true;
		case ARG_GRID: if (retval = parse_incangle(cmd, i)) break;
			continue;
		...
		case ARG_ORDER: if (retval = parse_order(p)) break;
			if ((pEXP->flags&EXP_USE_VDIST) && (!stricmp(p, "DA") || !stricmp(p, "AD"))) bUse_vdist |= 1;
			else if (!strchr(p, 'e') && !strchr(p, 'E')) bUse_vdist = 0;
			continue;

		case ARG_TAPE: if (retval = parse_tape(p)) break;
			if ((pEXP->flags&EXP_USE_VDIST) && !stricmp(p, "SS")) bUse_vdist |= 2;
			else bUse_vdist = 0;
			continue;
...
static void log_lrud_error(SRV_LRUD &lrud)
{
	int e;

	for (e = SRV_SIZNAME; e; e--) if (lrud.nam[e - 1] != ' ') break;
	lrud.nam[e] = 0;
	e = nLines;
	nLines = lrud.lineno;
	log_error("LRUD at station %s ignored. Facing direction can't be determined", lrud.nam);
	nLines = e;
}
...
{
	return pLastTo && pLastTo->az != LRUD_BLANK && pfx == pLastTo->pfx && !memcmp(nam, pLastTo->nam, SRV_SIZNAME);
}

static int parse_lrud(int i)
{
	//Called either from parse_polar() with (i==bLrudOnly)=0,
	//or from srv_Next() (prior to parse_polar) with (i==bLrudOnly = 1 or 2;
	//In both cases, cfg_argv[i] == *.. or <..
	...
	for (t = 0; t < 4; t++) {
		lruddat.dim[t] = dim[u->lrudseq[t]];
	}
	lruddat.az = dim[4]; //normally blank

	//Get the name associated with this LRUD.
	//
	//If this is an LRUD-Only line:
	//  If there are two distinct names specified (lrudOnly=2), it's still the case that
	//  we store only the FIRST name in lruddat (same as wall-srv.cpp). Only if
	//  u->lrudmeth==LRUD_F, when there are two names, will we reverse the L-R dimension 
	//  lines after checking this first name against pLastTo->nam/idx.
	//
	//  If u->lrudmeth==LRUD_F, it's probably an ending TO station LRUD and we assume it's
	//  pointing AWAY FROM pVec->nam[0] in the PREVIOUS polar vector, thereby using that az.
	//  This LRUD name should match pVec->nam[1] in that vector (stored in *pLastTo).
	//
	//  If u->lrudmeth==LRUD_T, it's probably a starting FROM station LRUD and we assume it's
	//  pointing AWAY FROM pVec->nam[0] in the NEXT polar vector, thereby using that az.
	//  This LRUD name should match pVec->nam[0] in that vector. To check this later, we'll
	//  now set lruddat_pending=lruddat and bLrudPending

	t = (u->lrudmeth < LRUD_T || bLrudOnly) ? 0 : 1;
	memcpy(lruddat.nam, pVec->nam[t], SRV_SIZNAME);
	lruddat.pfx = pVec->pfx[t];

	if (bLrudOnly && lruddat.az == LRUD_BLANK && (lruddat.dim[0] || lruddat.dim[1])) {
		//Common case: LRUD-only without the needed direction specified on the line --
		if (u->lrudmeth >= LRUD_T) {
			if (bLrudOnly > 1) {
				if (!bErrTwoNames) {
					log_error("Second name on LRUD-only line ignored when LRUD type is T");
					bErrTwoNames = TRUE;
				}
			}
			//Must be a starting station LRUD --
			//When processing the next vector we'll issue log_lrud_error() if
			//the FROM name fail to match name in lruddat_pending.nam[bLrudPending] --
			bLrudPending = bLrudOnly;
			lruddat_pending = lruddat; //will need the names and lineno
		}
		else {
			//Might be an ending statio LRUD-only --
			//Get direction from pLastTo --
			if (!LastToOK(lruddat.pfx, lruddat.nam)) {
				log_lrud_error(lruddat);
			}
			else {
				//This is an LRUD-only for the previous TO station.
				//The pLasto name and pfx match, and pLasto->az!=LRUD_BLANK --
				//lruddat.az=get_lrud_degrees(pLastTo->az); //don't show azimuth in SEF lrud
				if (bLrudOnly > 1) {
					//Backward compatibility -- two stations on a FROM-station LRUD-only line:
					//The name for this LRUD is also the TO station on this line --
					float fswap = lruddat.dim[0];
...
static int ParseRect(void)
{
	...
	if (bLatLong && bFixed) {
		if (!pEXP->pREF && set_charno(0)) return SRV_ERR_NODATUM;
		for (i = 0; i < 2; i++) pVec->xyz[i] = xyz[i];
		if ((e = srv_CB(FCN_CVTLATLON, (LPSTR)pVec)) && set_charno(0)) return e;
	...
	while ((e = cfg_GetLine()) >= 0) {
		...
		if (e) {
			...
			if (e != CMD_FIX) {
				...
				if (e != CMD_UNITS && e != CMD_DATE && (e<CMD_PREFIX || e>CMD_PREFIX3)) {
					if (*line_comment) {
		...
		//This may truncate ln and set pSEG=&segLine and pSEG->nPending > 0.
		//It also sets bLrudOnly to 0 (normal vec), 1 (one name), or 2 (two names).
		...
		if (bFixed) {
			...
			if (bLrudPending) {
				assert(u->lrudmeth >= LRUD_T);
				//Name in preceeding LRUD-only must match the current FROM name to avoid looging an error --
				if (bLrudOnly || lruddat_pending.pfx != pVec->pfx[0] || memcmp(lruddat_pending.nam, pVec->nam[0], SRV_SIZNAME))
					log_lrud_error(lruddat_pending);
				bLrudPending = FALSE;
			}
			...
			if (u->vectype == U_POLAR) {
				e = parse_polar();
				if (e == -1) continue; //ignore spray shot for now
			}
			else {
				e = ParseRect();
			}
...
static int PASCAL exp_Open()
{
	...
	*seg = *last_seg = 0;
	u->gridnorth = pEXP->pREF ? pEXP->pREF->conv : 0;
	...
	pSEG = &segMain;
	segMain_id = -1;
	segMain.nPending = segLine.nPending = 0;
...
static void outSEF_F(double d, int decs, BOOL bComma)
{
	char buf[32];
	char *p = buf;
	snprintf(p, 30, "%.*f", decs, d); p[30] = 0;
	if (strchr(p, '.')) {
		int i = strlen(p);
		while (i > 1 && p[i - 1] == '0') i--;
		if (p[i - 1] == '.') i--;
...
static void outSEF_LC(LPCSTR suffix)
{
	if (*line_comment) {
		outSEF("%s%s\r\n", line_comment, suffix ? "" : suffix);
		*line_comment = 0;
	}
}
...
static void outSEF_CC(BOOL bElimTitle /*=FALSE*/)
{
	if (pCC) {
		for (int i = 0; i < numCC; i++) {
			if (bElimTitle && !i && !stricmp(pCC[0], pEXP->title)) {
				continue;
			}
			outSEF("%s\r\n", pCC[i]);
		}
...
static bool IsFromToHdr(LPCSTR p)
{
	if (strlen(p) > 7 && !memcmp(p, "FROM", 4) && isspace(*(p += 4))) {
		while (isspace(*++p));
		if (*p++ == 'T' && *p++ == 'O' && isspace(*p)) return true;
	}
	else {
		while (isspace(*++p));
		if (*p++ == 'N' && *p++ == 'A' && *p++ == 'M '&& *p == 'E') return true;
	}
	return false;
}
...
static void addCC(const char *format, ...)
{
	...
	*buf = buf[255] = 0;
	...
	if (!*pbuf || IsFromToHdr(pbuf)) return;
...
static void outSEF_group(LPCSTR typ)
{
	if (filegroups) {
		DWORD dw = *(DWORD *)pVec->date;
		char date[12];
		*date = 0;
		if (typ[1] == 't' && (dw & 0xFFFFFF)) {
			sprintf(date, " %d-%02d-%02d", (dw >> 9) & 0x7FFF, (dw >> 5) & 0xF, dw & 0x1F);
		}
		outSEF("#%s >%s%s\r\n", typ, pEXP->title, date);
	}
	else {
...
static void outSEF_date()
{
	DWORD dw = *(DWORD *)pVec->date;
	// dw=(y<<9)+(m<<5)+d; //15:4:5
	if (dw & 0xFFFFFF) {
		outSEF("#date %02d/%02d/%d\r\n", (dw >> 5) & 0xF, dw & 0x1F, (dw >> 9) & 0x7FFF);
	}
}
...
static void Out_CT_Header(void)
{
	...
	if (bConvert) {
		...
		else if (bNoInc && u->tapemeth == TAPE_SS) outSEF_s("NOTE: Vectors were derived from a dive survey in Walls.\n");
	...
	if (bDateFirst) outSEF_date();
	outSEF_s("#decl "); outSEF_F(ct_decl, 4, 2);
	if (!bDateFirst) outSEF_date();
	...
	if (!bConvert) {
		...
		if (pEXP->flags&EXP_USE_ALLSEF) {
			if (u->incA) {
				outSEF_s("#fcc "); outSEF_F(u->incA / U_DEGREES, 4, 2);
			}
			if (u->incAB) {
				outSEF_s("#bcc "); outSEF_F(u->incAB / U_DEGREES, 4, 2);
			}
			if (u->incV) {
				outSEF_s("#fic "); outSEF_F(u->incV / U_DEGREES, 4, 2);
			}
			if (u->incVB) {
				outSEF_s("#bic "); outSEF_F(u->incVB / U_DEGREES, 4, 2);
			}
			if (u->incD) {
				outSEF_s("#mtc "); outSEF_F(u->incD*ct_scale, 4, 2);
			}
		}
	}
...
static void Out_CT_LrudOnly(LPCSTR pName)
{
	...
	outSEF(" *,%s,%s,*,", pName, pName);
	if (lruddat.az != LRUD_BLANK) outSEF_F(lruddat.az, 4, 1);
	else outSEF_s("*,");
	if (bConvert) outSEF_s("0,");
	else {
		if (bUse_vdist > 2) {
			outSEF_s("0,0,");
		}
	...
	for (int i = 0; i < 4; i++) {
		outSEF_FD(lruddat.dim[i], i < 3);
	}
	outSEF_ln();
}

static void end_cpvectors()
{
	assert(!ctvectors);
	outSEF_s("#endcpoint\r\n\r\n");
	cpvectors = 0;
...
static void end_ctsurvey()
{
	if (bLrudPending) log_lrud_error(lruddat_pending);
	bLrudOnly = bLrudPending = ct_islrud = 0;
	pLastTo = NULL;
	assert(!cpvectors);
...
static int Out_CT_Vector(void)
{
	...
	//We've just processed either a complete vector or an LRUD only line, with bLrudOnly!=0
	//In the latter case, we output the LRUD in lruddat and return.
	//Otherwise, output the vector and also data in lruddat (if ct_islrud!=0)

	if (!ctvectors || compare_uCT()) {
		if (ctvectors) {
			end_ctsurvey(); //sets ctvectors=0
		...
		bDateFirst = bDateSeen = bDeclSeen = false;
	...
	char nam1[EXP_MAX_NAMLEN + 4], nam2[EXP_MAX_NAMLEN + 4];

	if (e = get_SEF_name(nam1, 0)) return e;

	if (bLrudOnly) {
		Out_CT_LrudOnly(nam1);
		bLrudOnly = 0;
		return 0;
	}

	if (e = get_SEF_name(nam2, 1)) return e;
...
static int Out_CP_Vector(void)
{
	...
	if (!cpvectors || u->bFeet != last_u.bFeet || strcmp(last_seg, seg)) {
		...
		if (u->bFeet && (!pEXP->pREF || !pEXP->pREF->zone)) {
			ct_units = "feet";
			ct_scale = 1.0 / U_FEET;
		}
		else {
		...
		if (pEXP->pREF && pEXP->pREF->zone) {
			outSEF("#units utm\r\n");
			outSEF("#zone %u\r\n", pEXP->pREF->zone);
			outSEF("|datum: %s, conv: %.4f, georef: %.2fE %.2fN %.2fU\r\n", pEXP->pREF->datumname,
				pEXP->pREF->conv, pEXP->pREF->east, pEXP->pREF->north, pEXP->pREF->elev);
		}
		else {
			outSEF("#units %s\r\n", ct_units);
		}
	...
	if (!e) {
		outSEF(" %s,", name);
		outSEF_F(ct_scale*pVec->xyz[0], 4, 1);
		outSEF_F(ct_scale*pVec->xyz[1], 4, 1);
		outSEF_F(ct_scale*pVec->xyz[2], 4, 0);
		outSEF_ln();
		cpvectors++;
	}
	return e;
...
DLLExportC int PASCAL Export(EXPTYP *pexp)
{
	...
	switch (pEXP->fcn) {
	case EXP_OPEN:
		if (pEXP->fTotalLength != EXP_VERSION) e = EXP_ERR_VERSION;
		else {
			...
			if (!(fpout = fopen(strcpy(outpath, pEXP->path), "wb"))) e = EXP_ERR_INITSEF;
				...
				outSEF(";SEF Created by Walls v2.0 (dll v" EXP_VERSION_STR ") - %s\r\n", timestr());
			}
		}
		break;

	case EXP_SETOPTIONS:
		bDateFirst = bDeclSeen = bDateSeen = false;
	...
	case EXP_SURVEY:
		//Process survey data --
		bUse_vdist = 0;
		bLrudOnly = bLrudPending = bErrTwoNames = ct_islrud = 0;
		pLastTo = NULL;
