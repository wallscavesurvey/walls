...
static void Out_CT_LrudOnly(SRV_LRUD &lrud,LPCSTR pName);
...
static char lruddat_nam[EXP_MAX_NAMLEN+2];
static char lruddat_nam_pending[EXP_MAX_NAMLEN+2];

//typedef std::vector<SRV_LRUD> VEC_LRUD;
//typedef VEC_LRUD::iterator IT_LRUD;

//static VEC_LRUD vec_lrudf;
  ...
  for(i=0;i<cfg_argc;i++) {
    ...
    switch(cmd) {
      ...
      case ARG_ORDER  : if(retval=parse_order(p)) break;
		                if((pEXP->flags&EXP_USE_VDIST) && !stricmp(p,"DA")) bUse_vdist|=1;
...
static int parse_lrud(int i)
{
	...
	//if(u->lrudmeth<LRUD_T) bLrudPending=FALSE;
		...
		if(bLrudOnly) {
			...
			if(u->lrudmeth>=LRUD_T) {
			   ...
			   if(!LastToOK(lruddat.pfx,lruddat.nam)) {
				   ...
				   //Do not show azimuth in SEF lrud --
		   ...
		   if(LastToOK(pVec->pfx[0],pVec->nam[0])) lruddat.az=get_lrud_degrees(pLastTo->az);
			   ...
			   //let the importing program worry about this situation !!!
			   //bLrudPending=2;
		   }
		}
	}
	...
	if(bLrudPending) {
		lruddat_pending=lruddat;
		get_SEF_name(lruddat_nam_pending_nam,0);
	}
	else if(lruddat.az!=LRUD_BLANK) {
...
static int ParseRect(void)
{
  ...
  if(bLatLong && bFixed) {
	  ...
	  if((e=srv_CB(FCN_CVTLATLON, (LPSTR)pVec)) && set_charno(0)) {
	  	 log_error("Failure converting Lat/Long coordinates to UTM");
	  }
	  else {
...
static apfcn_i parse_polar(void)
{
  ...
  */
  ...
  if(bLrudPending) {
	  ...
	  if((lruddat.az==LRUD_BLANK) && (iPureVert ||
		  (bLrudPending==1 && (lruddat.pfx!=pVec->pfx[0] || memcmp(lruddat.nam,pVec->nam[0],SRV_SIZNAME))) ||
		  (bLrudPending==2 && !LastToOK(pVec->pfx[0], pVec->nam[0])))) {
		     log_lrud_error();
			 bLrudPending=FALSE;
	  }
	  else {
		  ...
		  lruddat_pending=lruddat;
		  get_SEF_name(lruddat_pending.nam,0);
	  }
  }
...
static void Out_CT_LrudOnly(SRV_LRUD &lrud,LPCSTR pName)
{
	...
	outSEF(" *,%s,%s,0,", pName, pName);
	if(lrud.az!=LRUD_BLANK) outSEF_F(lrud.az, 1);
	else outSEF_s("*,");
...
static void end_ctsurvey()
{
	assert(!cpvectors);
	if(bLrudOnly) {
		Out_CT_LrudOnly(lruddat,lruddat_nam_pending);
		bLrudOnly=0;
	}
...
static int Out_CT_Vector(void)
{
	...
	//Otherwise:
	// 1) if bLrudPending!=0, we output the LRUD line in lruddat_pending.
	// 2) Output the vector and data in lruddat (if ct_islrud!=0)
	...
	if(bLrudPending) {
		Out_CT_LrudOnly(lruddat_pending,lruddat_nam_pending);
	    bLrudPending=0;
    }
	else if(bLrudOnly) {
		Out_CT_LrudOnly(lruddat,lruddat_nam);
		bLrudOnly=0;
		return 0;
	}
