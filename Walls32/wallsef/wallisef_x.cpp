...
enum { SEF_ALLOWCOLONS=1, SEF_COMBINE=2, SEF_FLG_CHRS_PFX=4, SEF_FLG_CHRS_REPL=8 };
typedef SEF_PARAMS *PSEF_PARAMS;
enum { SEF_GETPARAMS };
...
static UINT linecount,filecount,veccount,fixcount,dircount,dirnest,surveycount,datacount;
static UINT repflags; // nchrs_repl, nchrs_pfx;
static int ln_len;
static bool bCombine,bExcluding,bLongNames;
static bool bUseUnits, bAllowColons;
static char cUnits;
...
DLLExportC int PASCAL ErrMsg(LPCSTR *pMsg,int code)
{
	...
	if(!code) {
		errMsg.Format("Conversion of %s to %s successful --\n\nFiles: %u  Surveys: %u  "
				"Vectors: %u  Fixed pts: %u  Length: %.2f m",
			trx_Stpnam(sefpath), prjname,
			filecount, surveycount, veccount, fixcount, lentotal);
	...
	if(code==SEF_ERR_ABORTED) strcpy(ln,errstr[code-1]);
		...
		wsprintf(ln,"Version %.2f of wallsef.dll required",psef_params->version);
	}
	else {
...
static void ElimSpc(char *p0)
{
	...
	for(p=p0; *p; p++) {
		...
		if(pp) {
			i=pp-pchrs_torepl;
			if(i==1 && bAllowColons || !i && p>p0) continue;
			*p=pchrs_repl[i];
			repflags|=(1<<i);
		}
	}
...
{
  double d;
  //For distances, for example,
  //cUnits - 'F' or 'M' as set by #units key in SEF, normally the units of numbers read from the file
  //*sefd_units - "Feet" or "Meters" that's being specified in Walls #units command
  //bUseUnits is true if we're assuming modern SEF in which case cUnits are the units of numbers read,
  //else the numbers are assumed to have feet units.
...
DLLExportC int PASCAL Import(LPSTR lpSefPath, LPSTR lpPrjPath, LPFN_IMPORT_CB pCB)
{
  ...
  trx_Stcext(prjpath,lpPrjPath,"wpj",_MAX_PATH);
  ...
  pathname=pathtail=trx_Stpnam(prjpath);
