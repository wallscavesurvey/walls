...
static bool bMakFile,bWarn,bUsePrefix2,bCombine,bAllowColons,bLongNames,bColonsAccepted;
...
static int open_fpto(UINT idx)
{
	...
	if(!srvidx) {
		...
		//open flag == 1
		//if open,   status= 9 (inherited), 2569 0xA09(own ref), 1353 0x549 (unspecified)
		int status=9;
		if(bMakFile) {
			if(pgeo->gZ) {
				if(pgeo->gD!=refdatum || pgeo->gZ!=refzone) {
					status=pfix?2569:1353;
				}
			}
...
_first:
	...
	fprintf(fppr,
	     ".SURVEY\t%s%s\n"
		 ".NAME\t%s\n"
		 ".STATUS\t8\n", bHasFixedPts?datbasename:survey, bHasFixedPts?" (Fixed Pts)":"", srvname);
	...
	outf(";%s\n;%s - Source Compass file dated %s\n", survey, datfullname, get_filetime(ln, vDatName[idx]));
	outlines+=2;
    ...
    while(e>=0) {
         ...
         switch(slinecount) {
           ...
           case 1:	if(!*p) p="Untitled Survey";
					strcpy(formatstr,p);   //cave name
					break;

           case 2:  *survey=0;
                    if(!out_survey_fld(p,"NAME")) { //fills survey[]
						return 0;
					}
					if(!bCombine && fpto) {
						fclose(fpto);
...
static int process_dat(UINT idx)
{
	...
	strncpy(datname, pPath, sizeof(datname)-1);
	...
	*trx_Stpext(datbasename)=0;
	...
	bHasFixedPts = pgeo && !pgeo->vFix.empty();

	//process first survey
	e=copy_hdr(idx); //updates project file (fppr), creatung it if necessary
